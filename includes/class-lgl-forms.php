<?php
/**
 * LGL Forms — Finance Calculator, Enquiry, and Reserve forms
 * Integrated directly into the LGL Shortcodes plugin.
 * Hooks into the existing LGL Settings admin panel and single-lgl.php template.
 */
...
class LGL_Forms {
	...
	public function add_submenu_pages() {
		add_submenu_page( 'lgl-settings', __( 'Finance Calculator Form', 'lgl-shortcodes' ),  __( 'Finance Calculator', 'lgl-shortcodes' ),   'manage_options', 'lgl-finance-form',  [ $this, 'page_finance' ] );
		add_submenu_page( 'lgl-settings', __( 'Enquiry Form Builder', 'lgl-shortcodes' ),      __( 'Enquiry Form', 'lgl-shortcodes' ),         'manage_options', 'lgl-enquiry-form',  [ $this, 'page_enquiry' ] );
		add_submenu_page( 'lgl-settings', __( 'Reserve Form Builder', 'lgl-shortcodes' ),      __( 'Reserve Form', 'lgl-shortcodes' ),         'manage_options', 'lgl-reserve-form',  [ $this, 'page_reserve' ] );
	...
	public function admin_assets( $hook ) {
		$lgl_form_pages = [
			'lgl-settings_page_lgl-finance-form',
			'lgl-settings_page_lgl-enquiry-form',
			'lgl-settings_page_lgl-reserve-form',
		];
		$is_submission_page = in_array( get_current_screen()->post_type ?? '', [ 'lgl_enquiry_sub', 'lgl_reserve_sub' ], true );

		if ( ! in_array( $hook, $lgl_form_pages, true ) && ! $is_submission_page ) return;
	...
	private function admin_css() {
		...
		.lgl-field-row{display:flex;align-items:stretch;border-bottom:1px solid #eee;background:#fff}
		...
		.lgl-field-actions{margin-left:auto;display:flex;align-items:flex-end}
		.lgl-rm-field{color:#d63638!important;text-decoration:none;padding:4px;border-radius:3px}
	...
	public function page_finance() {
		...
		$this->notice();
		?>
		<div class="wrap lgl-form-builder-wrap">
			<h1><?php _e( 'Finance Calculator Settings', 'lgl-shortcodes' ); ?></h1>
					...
					<div>
						<div class="lgl-fbl-section">
							...
							<table class="form-table" style="margin:0">
								<tr><th><label for="fc_button_text"><?php _e( 'Button Text', 'lgl-shortcodes' ); ?></label></th>
								<td><input type="text" id="fc_button_text" name="button_text" value="<?php echo esc_attr( $s['button_text'] ?? 'Finance Calculator' ); ?>" class="regular-text"></td></tr>
								<tr><th><label for="fc_title"><?php _e( 'Modal Title', 'lgl-shortcodes' ); ?></label></th>
								<td><input type="text" id="fc_title" name="title" value="<?php echo esc_attr( $s['title'] ?? 'Finance Calculator' ); ?>" class="large-text"></td></tr>
								<tr><th><label for="fc_subtitle"><?php _e( 'Subtitle', 'lgl-shortcodes' ); ?></label></th>
								<td><input type="text" id="fc_subtitle" name="subtitle" value="<?php echo esc_attr( $s['subtitle'] ?? '10.90% available, calculate the cost of your caravan or motorhome' ); ?>" class="large-text"></td></tr>
							</table>
						</div>
						<div class="lgl-fbl-section">
							<h3><?php _e( 'Financial Parameters', 'lgl-shortcodes' ); ?></h3>
							<table class="form-table" style="margin:0">
								<tr><th><label for="fc_apr"><?php _e( 'Representative APR (%)', 'lgl-shortcodes' ); ?></label></th>
								<td><input type="number" id="fc_apr" name="apr" value="<?php echo esc_attr( $s['apr'] ?? '10.90' ); ?>" step="0.01" min="0" class="small-text"> %
								<p class="description"><?php _e( 'Used for monthly repayment calculations.', 'lgl-shortcodes' ); ?></p></td></tr>
								<tr><th><label for="fc_fee"><?php _e( 'Option to Purchase Fee (£)', 'lgl-shortcodes' ); ?></label></th>
								<td>£ <input type="number" id="fc_fee" name="purchase_fee" value="<?php echo esc_attr( $s['purchase_fee'] ?? '10' ); ?>" step="1" min="0" class="small-text">
								<p class="description"><?php _e( 'Added to the final payment (e.g. £10 Option To Purchase Fee).', 'lgl-shortcodes' ); ?></p></td></tr>
								<tr><th><label for="fc_dur"><?php _e( 'Duration Options', 'lgl-shortcodes' ); ?></label></th>
								<td><textarea id="fc_dur" name="durations" rows="6" class="large-text"><?php echo esc_textarea( $s['durations'] ?? "1 Year\n2 Years\n3 Years\n4 Years\n5 Years" ); ?></textarea>
								<p class="description"><?php _e( 'One option per line. e.g. "1 Year", "18 Months".', 'lgl-shortcodes' ); ?></p></td></tr>
							</table>
						</div>
						<div class="lgl-fbl-section">
					...
					<div>
						...
						<div class="lgl-fbl-section">
							...
							<ul class="lgl-help-list">
								<li>Cash Price</li><li>Deposit</li><li>Total Amount of Credit</li>
								<li>Agreement Duration</li><li>Monthly Repayments of</li>
								<li>Total Amount Repayable</li><li>Purchase Fee</li>
								<li>Interest Rate (flat)</li><li>Representative APR</li><li>Monthly Payment*</li>
							</ul>
						</div>
					</div>
				</div>
	...
	private function render_form_builder_page( $type, $s ) {
		...
		?>
		<div class="wrap lgl-form-builder-wrap">
			<h1><?php echo $type === 'enquiry' ? __( 'Enquiry Form Builder', 'lgl-shortcodes' ) : __( 'Reserve Form Builder', 'lgl-shortcodes' ); ?></h1>
				...
				<div class="lgl-fbl-layout">
					<div>
						...
						<div class="lgl-fbl-section">
							...
							<table class="form-table" style="margin:0">
								<tr><th><label><?php _e( 'Button Text', 'lgl-shortcodes' ); ?></label></th>
								<td><input type="text" name="button_text" value="<?php echo esc_attr( $s['button_text'] ?? '' ); ?>" class="regular-text"></td></tr>
								<?php if ( $type === 'reserve' ) : ?>
								<tr><th><label><?php _e( '"Reserved" Button Text', 'lgl-shortcodes' ); ?></label></th>
								<td><input type="text" name="reserved_button_text" value="<?php echo esc_attr( $s['reserved_button_text'] ?? 'Reserved' ); ?>" class="regular-text">
								<p class="description"><?php _e( 'Shown on the button after the vehicle is reserved.', 'lgl-shortcodes' ); ?></p></td></tr>
								<?php endif; ?>
								<tr><th><label><?php _e( 'Modal Title', 'lgl-shortcodes' ); ?></label></th>
								<td><input type="text" name="title" value="<?php echo esc_attr( $s['title'] ?? '' ); ?>" class="large-text"></td></tr>
								<tr><th><label><?php _e( 'Submit Button Text', 'lgl-shortcodes' ); ?></label></th>
								<td><input type="text" name="submit_text" value="<?php echo esc_attr( $s['submit_text'] ?? '' ); ?>" class="regular-text"></td></tr>
								<tr><th><label><?php _e( 'Success Message', 'lgl-shortcodes' ); ?></label></th>
								<td><textarea name="success_message" rows="3" class="large-text"><?php echo esc_textarea( $s['success_message'] ?? '' ); ?></textarea></td></tr>
								<?php if ( $type === 'reserve' ) : ?>
								<tr><th><label><?php _e( 'Default Reserve Mode', 'lgl-shortcodes' ); ?></label></th>
								<td>
					...
					<div>
						...
						<?php endif; ?>
						<div class="lgl-fbl-section">
							<h3><?php _e( 'Submissions', 'lgl-shortcodes' ); ?></h3>
							<?php if ( $type === 'enquiry' ) : ?>
							<a href="<?php echo admin_url( 'edit.php?post_type=lgl_enquiry_sub' ); ?>" class="button"><?php _e( 'View Enquiry Submissions', 'lgl-shortcodes' ); ?></a>
							<?php else : ?>
							<a href="<?php echo admin_url( 'edit.php?post_type=lgl_reserve_sub' ); ?>" class="button"><?php _e( 'View Reserve Submissions', 'lgl-shortcodes' ); ?></a>
							<?php endif; ?>
						</div>
					</div>
				</div>
			...
			<div class="lgl-field-body">
				<div class="lgl-field-top">
					...
					<div class="lgl-field-actions">
						<button type="button" class="lgl-rm-field button-link" title="Remove"><span class="dashicons dashicons-trash"></span></button>
					</div>
				</div>
	...
	public function save_finance() {
		...
		wp_redirect( admin_url( 'admin.php?page=lgl-finance-form&saved=1' ) ); exit;
	}
	...
	public function save_enquiry() {
		...
		wp_redirect( admin_url( 'admin.php?page=lgl-enquiry-form&saved=1' ) ); exit;
	}
	...
	public function save_reserve() {
		...
		wp_redirect( admin_url( 'admin.php?page=lgl-reserve-form&saved=1' ) ); exit;
	}
	...
	public function render_product_meta_box( $post ) {
		...
		(function($){
			...
			function upd(){ $('#lgl-vf-desc').text(descs[$('#lgl-vf-mode').val()]||''); }
			$('#lgl-vf-mode').on('change',upd); upd();
		})(jQuery);
		</script>
		<?php
	}
	...
	public function render_sub_detail( $post ) {
		...
		if ( ! $data ) { echo '<p><em>' . __( 'No data recorded.', 'lgl-shortcodes' ) . '</em></p>'; return; }
	...
	public function ajax_submit_enquiry() {
		...
		$this->notify_admin( 'enquiry', $data, $product_id );
	...
	private function notify_admin( $type, $data, $product_id ) {
		$email = get_option( 'admin_email' );
		if ( ! $email ) return;
		$product = $product_id ? get_the_title( $product_id ) : 'Unknown Product';
		$name    = trim( ( $data['first_name'] ?? '' ) . ' ' . ( $data['last_name'] ?? '' ) );
		$subj    = $type === 'enquiry' ? "New Enquiry: {$name} — {$product}" : "New Reservation: {$name} — {$product}";
	...
	/**
	 * Render a single form field HTML (used inside modal templates).
	 */
	...
	private function notice() {
		if ( isset( $_GET['saved'] ) ) echo '<div class="notice notice-success is-dismissible"><p>' . __( 'Settings saved.', 'lgl-shortcodes' ) . '</p></div>';
	}
}
