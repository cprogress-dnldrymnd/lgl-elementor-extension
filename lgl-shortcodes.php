...
define('LGL_SHORTCODES_VERSION', '4.5.9');
...
if (! class_exists('LGL_Shortcodes')) {
    ...
    class LGL_Shortcodes
    {
        ...
        public function __construct()
        {
            ...
            // New AJAX endpoints for compare functionality
        ...
        /**
         * Overrides WordPress core restrictions to permit SVG file uploads.
         * * @param array $mimes Array of natively allowed MIME types.
         * @return array Modified array including image/svg+xml.
         */
        public function allow_svg_uploads($mimes)
        {
            // Authorize the SVG file extension and its exact MIME type
...
?>
            ...
            switch ($args['type']) {
                ...
                case 'select_page':
                    ...
                    wp_dropdown_pages(array(
                        'name'              => 'lgl_settings[' . esc_attr($id) . ']',
                        'id'                => 'lgl_settings[' . esc_attr($id) . ']',
        ...
        {
        ?>
            <script src="http://cdnjs.cloudflare.com/ajax/libs/iframe-resizer/3.6.0/iframeResizer.min.js"></script>
            <script>
                iFrameResize({
                    log: false,
        ...
        public function render_shortcode($atts, $content = null, $shortcode_tag = '')
        {
            ...
            if ($shortcode_tag == 'lgl_breadcrumbs') {
                $attributes_arr['style'] = 'dark'; // Accepts 'dark' or 'light'
            }
        ...
        public function ajax_get_compare_table()
        {
            ...
            if (!$query->have_posts()) {
                wp_send_json_error('Selected vehicles are no longer available.');
            }
        ...
        public function ajax_get_filter_options()
        {
            ...
            $data_post_type = isset($_POST['post_type']) ? sanitize_text_field($_POST['post_type']) : 'caravan';
            $form_data = array();
            if (!empty($_POST['form_data'])) {
                parse_str(stripslashes($_POST['form_data']), $form_data);
            }
                        ...
                        if ($t->parent == 0) {
                            ...
                            if (!isset($seen_makes[$t->parent])) {
                                ...
                                if ($parent_term && !is_wp_error($parent_term)) {
                                    $makes[] = array('id' => $parent_term->slug, 'text' => $parent_term->name);
                                    $seen_makes[$parent_term->term_id] = true;
                                }
                            }
                        }
                    }
        ...
        public static function get_search_results_data($post_type = 'caravan', $form_data = array(), $paged = 1, $posts_per_page = 9, $is_carousel = false, $style = 'style-1', $is_featured = false)
        {
            ...
            if ($query->have_posts()) {
                while ($query->have_posts()) {
                    $query->the_post();
                    if ($is_carousel) {
                        echo '<div class="swiper-slide vehicle-slide">';
                    }
        ...
        /**
         * 2. Injects the HTML <img> tag when templates call the_post_thumbnail() or get_the_post_thumbnail()
         */
        ...
        /**
         * 3. Injects the raw URL when templates or Elementor call get_the_post_thumbnail_url()
         */
        public function fallback_post_thumbnail_url($url, $post_id, $size)
        {
            if (empty($url)) {
                $post_type = get_post_type($post_id);
                if (in_array($post_type, array('caravan', 'motorhome', 'campervan'))) {
